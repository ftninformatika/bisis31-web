  ...
  static {
    try {
      ...
      format = FormatFactory.getFormat(PubTypes.class.getResource(
          "/com/gint/app/bisis/common/format/spec/unimarc.xml").openStream());
      
      log.info("Loading publication type definitions");
      String dirName = "/com/gint/app/bisis/common/format/spec";
      String[] files = FileUtils.listFiles(PubTypes.class, dirName);
      for (int i = 0; i < files.length; i++) {
        if (files[i].endsWith(".xml") && !files[i].endsWith("unimarc.xml")) {
